...
class Library:
    ...
    def get_next_book_id(self):
        if not self.books:  # If the library is empty
            return 1
        return self.books[-1].id + 1  # Return the next id
...
if __name__ == '__main__':
    ...
    library_with_books = Library(books=list_books)  # Initialize library with books
    print(library_with_books.get_next_book_id())  # Check the next id for a non-empty library

    print(library_with_books.get_index_by_book_id(1))  # Check index of book with id = 1